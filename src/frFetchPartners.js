...
//   const pS = AppConfig.PARTNERS_SHEET_SETTINGS;
...
//   const requestUrl = `${AppConfig.FREEE_BASE_URL}/api/1/partners?company_id=${ourCompanyId}&limit=3000`;
...
//       const partnarsData = partners.map(p => [p.name, p.id]);
...
//   const concatValues = partnersData.map(row => {
//     const name = row[pS.nameCol - 1];
//     const id   = row[pS.idCol   - 1];
//     return [`[${id}]  ${name}`];
//   });
...
//   const pS = AppConfig.PARTNERS_SHEET_SETTINGS;
//   const accessToken = getFreeeOAuth2Service().getAccessToken();
//   const requestUrl = `${AppConfig.FREEE_BASE_URL}/api/1/companies`;
...
//       partnerSheet.getRange(pS.ourCompanyListRow, pS.ourCompanyListCol, companyData.length, 2)
